...
test_that("tmr_round", {
  tmr <- tmr_timer()
  expect_identical(tmr_round(tmr), tmr)
  tmr <- tmr_round(tmr_start(tmr))
  expect_lte(tmr_elapsed(tmr), 1L)
  expect_gte(tmr_elapsed(tmr), 0L)
  expect_true(tmr_is_started(tmr))
  Sys.sleep(1)
  expect_gte(tmr_elapsed(tmr), 1L)
  expect_lte(tmr_round(tmr), 2L)
  tmr <- tmr_round(tmr_stop(tmr))
...
test_that("tmr_floor", {
  ...
  tmr <- tmr_floor(tmr_start(tmr))
  expect_lte(tmr_floor(tmr), 1L)
  expect_gte(tmr_floor(tmr), 0L)
  expect_true(tmr_is_started(tmr))
  Sys.sleep(1)
  expect_gte(tmr_floor(tmr), 1L)
  expect_lte(tmr_floor(tmr), 2L)
  tmr <- tmr_floor(tmr_stop(tmr))
  expect_identical(tmr, tmr_elapsed(tmr))
})
...
test_that("tmr_ceiling", {
  ...
  expect_true(tmr_is_started(tmr))
  Sys.sleep(1)
  expect_gte(tmr_elapsed(tmr), 2L)
  expect_lte(tmr_ceiling(tmr), 3L)
  tmr <- tmr_ceiling(tmr_stop(tmr))
  expect_identical(tmr, tmr_elapsed(tmr))
})
